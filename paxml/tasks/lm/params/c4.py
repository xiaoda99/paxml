...
class DensePrePostNorm: 
  DENSE_BIAS_INIT_METHOD = 'zeros'
  USE_DENSE_PRE_NORM = True
  DYNAMIC_DENSE_ADD_RESIDUAL = True
...
@experiment_registry.register
class PileLlamaXL2Kx4x256x1SpeedTest(_TrainConfig2Kx4x256x1, PileDataParams, PythiaInit, _LlamaXL):
  QUERY_CHUNK_SIZE = None  # v5p-128: 0.547
  # QUERY_CHUNK_SIZE = 512 
  # pass # v5p

@experiment_registry.register
class PileLlamaXL4Kx2x256x1SpeedTest(PileLlamaXL2Kx4x256x1SpeedTest):
  PERCORE_BATCH_SIZE = 2
  MAX_SEQ_LEN = 4097

@experiment_registry.register
class PileMUDDLlamaXL2Kx4x256x1SpeedTest(MultiWayDynamicDenseConfig, PileLlamaXL2Kx4x256x1SpeedTest): 
  # QUERY_CHUNK_SIZE = 512
  DYNAMIC_DENSE_HIDDEN_ROUND = True # v5p-128: 0.491
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [4]
  HIDDEN_DIMS = [round(5504 * (i/23 +0.5) / 128) * 128 for i in range(24)] 

@experiment_registry.register
class PileMUDDLlamaXL4Kx2x256x1SpeedTest(PileMUDDLlamaXL2Kx4x256x1SpeedTest):
  PERCORE_BATCH_SIZE = 2
  MAX_SEQ_LEN = 4097

@experiment_registry.register
class PileMUDDLlamaXL2Kx4x256x1SpeedTestPrePostNorm(DensePrePostNorm, PileMUDDLlamaXL2Kx4x256x1SpeedTest):
  pass

@experiment_registry.register
class PileMUDDLlamaXL2Kx4x256x1SpeedTestPrePostNormQD1KD2(DensePrePostNorm, PileMUDDLlamaXL2Kx4x256x1SpeedTest):
  DENSE_KEY_DILATION = 2
  DENSE_QUERY_DILATION = 1

@experiment_registry.register
class PileMUDDLlamaXL2Kx4x256x1SpeedTestQD2KD2(PileMUDDLlamaXL2Kx4x256x1SpeedTest):
  DENSE_KEY_DILATION = 2
  DENSE_QUERY_DILATION = 2

@experiment_registry.register
class PileMUDDLlamaXL2Kx4x256x1SpeedTestQD2KD1(PileMUDDLlamaXL2Kx4x256x1SpeedTest):
  DENSE_KEY_DILATION = 1
  DENSE_QUERY_DILATION = 2

@experiment_registry.register
class PileMUDDLlamaXL2Kx4x256x1SpeedTestWindow8(PileMUDDLlamaXL2Kx4x256x1SpeedTest):
  DENSE_KEY_WINDOW = 8

@experiment_registry.register
class PileLlamaXLDenseformer2Kx4x256x1SpeedTest(PileLlamaXL2Kx4x256x1SpeedTest): 
  DENSE_CONN = True
  REMAT = True
  USE_REPEATED_LAYER = False

@experiment_registry.register
class PileLlamaXLDynamicDenseformer2Kx4x256x1SpeedTest(_DynamicDenseConfig, PileLlamaXL2Kx4x256x1SpeedTest):
  pass 

@experiment_registry.register
class PileLlama3B2Kx4x256x1SpeedTest(PileLlama3B2Kx8x128x1):
  # QUERY_CHUNK_SIZE = None # v5p: 0.326, v4:
  QUERY_CHUNK_SIZE = 512 # v5p: , v4:
  PERCORE_BATCH_SIZE = 4
  ICI_MESH_SHAPE = [1, 256, 1]

@experiment_registry.register
class PileMUDDLlama3B2Kx4x256x1SpeedTest(MultiWayDynamicDenseConfig, PileLlama3B2Kx4x256x1SpeedTest):
  DYNAMIC_DENSE_HIDDEN_ROUND = True # v5p-128: 0.274, v4:
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 31 + [4]
  HIDDEN_DIMS = [round(6912 * (i/31 +0.5) / 128) * 128 for i in range(32)] 

@experiment_registry.register
class PileLlama7B2Kx4x256x1SpeedTest(PileLlama7B2Kx4x256x1):
  # QUERY_CHUNK_SIZE = None # v5p-128: 0.1584, v4: 
  QUERY_CHUNK_SIZE = 512
  PERCORE_BATCH_SIZE = 4
  ICI_MESH_SHAPE = [1, 256, 1]

@experiment_registry.register
class PileMUDDLlama7B2Kx4x256x1SpeedTest(MultiWayDynamicDenseConfig, PileLlama7B2Kx4x256x1SpeedTest):
  DYNAMIC_DENSE_HIDDEN_ROUND = True # v5p-128: 0.1514
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 31 + [4]
  HIDDEN_DIMS = [round(11008 * (i/31 +0.5) / 128) * 128 for i in range(32)] 

@experiment_registry.register
class PileLlama3B4Kx2x256x1SpeedTest(PileLlama3B2Kx4x256x1SpeedTest):
  PERCORE_BATCH_SIZE = 2
  MAX_SEQ_LEN = 4097

@experiment_registry.register
class PileMUDDLlama3B4Kx2x256x1SpeedTest(PileMUDDLlama3B2Kx4x256x1SpeedTest):
  PERCORE_BATCH_SIZE = 2
  MAX_SEQ_LEN = 4097

@experiment_registry.register
class PileLlama7B4Kx2x256x1SpeedTest(PileLlama7B2Kx4x256x1SpeedTest):
  PERCORE_BATCH_SIZE = 2
  MAX_SEQ_LEN = 4097

@experiment_registry.register
class PileMUDDLlama7B4Kx2x256x1SpeedTest(PileMUDDLlama7B2Kx4x256x1SpeedTest):
  PERCORE_BATCH_SIZE = 2
  MAX_SEQ_LEN = 4097

@experiment_registry.register
class PileLlama13B2Kx4x256x1SpeedTest(PileLlama13B2Kx4x256x1):
  QUERY_CHUNK_SIZE = None # v5p-256: , v4:

@experiment_registry.register
class PileMUDDLlama13B2Kx4x256x1SpeedTest(MultiWayDynamicDenseConfig, PileLlama13B2Kx4x256x1SpeedTest):
  DYNAMIC_DENSE_HIDDEN_ROUND = True # v5p-256: v4:
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 39 + [4]
  HIDDEN_DIMS = [round(13824 * (i/39 +0.5) / 128) * 128 for i in range(40)] 
...
@experiment_registry.register
class PileLlamaXLHyperConnAttnTanh(PileLlamaXLHyperConnAttn):
...
@experiment_registry.register
class PileMUDDLlamaXL(MultiWayDynamicDenseConfig, PileLlamaXL):
  DYNAMIC_DENSE_HIDDEN_ROUND = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [4]

@experiment_registry.register
class PileMUDDLlamaXLPlus(MultiWayDynamicDenseConfig, PileLlamaXL):
  DYNAMIC_DENSE_HIDDEN_ROUND = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [4]
  HIDDEN_DIMS = [round(5504 * (i/23 +0.5) / 128) * 128 for i in range(24)] 
...
class PileMUDDLlamaXLPlusTall(PileMUDDLlamaXLPlus):
  ...
  MODEL_DIMS = 1024
  NUM_HEADS = 16
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 95 + [4]
  HIDDEN_DIMS = [round(2816 * (i/95 +0.5) / 128) * 128 for i in range(96)] 
  CHECKPOINT_EVERY_N_STEPS = 1000
  TENSORSTORE_USE_OCDBT = True
  ASYNC_CHECKPOINT = True

@experiment_registry.register
class PileMUDDLlamaXLPlusHigh(PileMUDDLlamaXLPlus):
  NUM_LAYERS = 42
  MODEL_DIMS = 1536
  NUM_HEADS = 24
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 41 + [4]
  VARIABLE_NORM_SUMMARY = True 
  HIDDEN_DIMS = [round(4096 * (i/41 +0.5) / 128) * 128 for i in range(42)] 
...
@experiment_registry.register
class PileMUDDLlamaXLPlusNotRoundDynHidDim(MultiWayDynamicDenseConfig, PileLlamaXL):
  DYNAMIC_DENSE_HIDDEN_ROUND = False
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [4]
  HIDDEN_DIMS = [round(5504 * (i/23 +0.5) / 128) * 128 for i in range(24)] 

@experiment_registry.register
class PileMUDDLlamaXLPlusNotRoundDynHidDimLastExpd64(PileMUDDLlamaXLPlusNotRoundDynHidDim):
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [64]

@experiment_registry.register
class PileMUDDLlamaXLVectorScaleKeywise(PileMUDDLlamaXL):
  DENSE_VECTOR_SCALE_KEYWISE = True
  DENSE_VECTOR_SCALE = True

@experiment_registry.register
class PileMUDDLlamaXLAdjustMLPDim(PileMUDDLlamaXL):
  HIDDEN_DIMS = [int(5504 * (i/23 +0.5)) for i in range(24)]  # num_layers=24, hid_dim=5504 0.5 -> 1.5

# HIDDEN_DIMS = [round(5504 * (i/23 +0.5) / 128) * 128 for i in range(24)] 
@experiment_registry.register
class PileMUDDLlamaXLAdjustHead(PileMUDDLlamaXL):
  NUM_HEADS = [16] * 8 + [32] * 8 + [48] * 8
  DIMS_PER_HEAD = 64

@experiment_registry.register
class PileMUDDLlamaXLAdjustHeadSoft(PileMUDDLlamaXL):
  NUM_HEADS = [24] * 8 + [32] * 8 + [40] * 8
  DIMS_PER_HEAD = 64

@experiment_registry.register
class PileMUDDLlamaXLAdjustHeadGradual(PileMUDDLlamaXL): 
  NUM_HEADS = [round(32 * (i/23 +0.5)) for i in range(24)] # 0.5 -> 1.5
  DIMS_PER_HEAD = 64

@experiment_registry.register
class PileMUDDLlamaXLAdjustHeadGradualSoft(PileMUDDLlamaXL):
  NUM_HEADS = [round(32 * (i/23/2 +0.75)) for i in range(24)] # 0.75 -> 1.25
  DIMS_PER_HEAD = 64

@experiment_registry.register
class PileMUDDLlamaXLAdjustHeadAdjustMLPDim(PileMUDDLlamaXLAdjustHead):
  HIDDEN_DIMS = [int(5504 * (i/23 +0.5)) for i in range(24)]

@experiment_registry.register
class PileMUDDLlamaXLAdjustHeadAdjustMLPDimLastExpd64(PileMUDDLlamaXLAdjustHeadAdjustMLPDim):
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [64] 

@experiment_registry.register
class PileMUDDLlamaXLQVM(PileMUDDLlamaXL):
  DYNAMIC_DENSE_TYPE = 'qvm'
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [3]

@experiment_registry.register
class PileMUDDLlamaXLQKVGM(PileMUDDLlamaXL):
  DYNAMIC_DENSE_TYPE = 'qkvgm'
  DYNAMIC_DENSE_SEPERATE_GATING_LN = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [5]
...
@experiment_registry.register
class PileDCLlamaXLDWDDHeadActGELU(PileDCLlamaXLDWDD):
  head_act_activation_cls = layers.GELU
  head_act_stop_grad = False

@experiment_registry.register
class PileDCLlamaXLDWDDHeadActGELUBias(PileDCLlamaXLDWDDHeadActGELU):
  USE_HEAD_ACT_BIAS = True
  SKIP_HEAD_ACT_BIAS_DECAY = True
...
class PileMUDDLlamaLargePlusHigh(PileMUDDLlamaLargePlus):
  ...
  MODEL_DIMS = 1280
  NUM_HEADS = 20
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 33 + [4]
  VARIABLE_NORM_SUMMARY = True
  HIDDEN_DIMS = [round(3456 * (i/33 +0.5) / 128) * 128 for i in range(34)] 
...
@experiment_registry.register
class PileMUDDLlamaLargePlusHighPrePostNormScaleDw(PileMUDDLlamaLargePlusHighPrePostNorm):
  DYNAMIC_DENSE_SCALE_DW = True
...
# @experiment_registry.register
# class PileLlamaMediumDebug(PileLlamaMedium):
#   PERCORE_BATCH_SIZE = 8 // 4
#   NUM_LAYERS = 1

# @experiment_registry.register
# class PileLlamaMediumDebug3(PileLlamaMediumDebug):
#   SAVE_ON_STEPS = [100,300, 500, 700]
#   CHECKPOINT_MAX_TO_KEEP = 2
#   CHECKPOINT_EVERY_N_STEPS = 100
#   TENSORSTORE_USE_OCDBT = True
#   ASYNC_CHECKPOINT = True
...
@experiment_registry.register
class PileLlamaMediumTall(PileLlamaMedium):
  NUM_LAYERS = 96
  MODEL_DIMS = 512
  NUM_HEADS = 8
  HIDDEN_DIMS = 1280
...
@experiment_registry.register
class PileLlamaMediumTallMatchParam(PileLlamaMediumTall):
  HIDDEN_DIMS = 1408

@experiment_registry.register
class PileLlamaMediumTallMatchParamHybridNorm(PileLlamaMediumTallMatchParam):
  NORM_POLICY = "primer_hybrid"

@experiment_registry.register
class PileLlamaMediumIncrHidDimAcrossLayerSteep(PileLlamaMedium):
  REMAT = True
  USE_REPEATED_LAYER = False
  HIDDEN_DIMS = [int(2816 * (i/23 * 1.6 +0.2)) for i in range(24)] # 0.2 -> 1.8

@experiment_registry.register
class PileLlamaMediumIncrHidDimAcrossLayer(PileLlamaMedium):
  REMAT = True
  USE_REPEATED_LAYER = False
  HIDDEN_DIMS = [int(2816 * (i/23 +0.5)) for i in range(24)]  # 0.5 -> 1.5
...
@experiment_registry.register
class PileLlamaMediumDynamicDensePlus(_DynamicDenseConfig, PileLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_ROUND = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [4]
  HIDDEN_DIMS = [round(2816 * (i/23 +0.5) / 128) * 128 for i in range(24)]
...
@experiment_registry.register
class PileMUDDPythiaMediumx6p7BTokenPlus(PileMUDDPythiaMediumx6p7BToken):
  DYNAMIC_DENSE_HIDDEN_ROUND = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [4]
  HIDDEN_DIMS = [round(4096 * (i/23 +0.5) / 128) * 128 for i in range(24)] 
...
@experiment_registry.register
class PileMUDDPythia3BPlus(MultiWayDynamicDenseConfig, PilePythia3B128x1FixRot):
  DYNAMIC_DENSE_HIDDEN_ROUND = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 31 + [4]
  HIDDEN_DIMS = [round(10240 * (i/31 +0.5) / 128) * 128 for i in range(32)] 

  CHECKPOINT_EVERY_N_STEPS = 500
  SAVE_ON_STEPS = list(range(3000, 153000, 10000))
  CHECKPOINT_MAX_TO_KEEP = 2
  PERCORE_BATCH_SIZE = 4 # 4
  ICI_MESH_SHAPE = [1, 256, 1] #  256 v5p-64
  QK_NORM = True
...
@experiment_registry.register
class PileMUDDPythia3BPlusOcdbt(PileMUDDPythia3BPlus):
  CHECKPOINT_EVERY_N_STEPS = 500
  TENSORSTORE_USE_OCDBT = True
  ASYNC_CHECKPOINT = True
...
class PileMUDDPythiaXLPlusOcdbt(MultiWayDynamicDenseConfig, PilePythiaXL128x1FixRot):
  DYNAMIC_DENSE_HIDDEN_ROUND = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [4] 
  HIDDEN_DIMS = [round(8192 * (i/23 +0.5) / 128) * 128 for i in range(24)] 
  SAVE_ON_STEPS = list(range(3000, 153000, 10000))
  CHECKPOINT_MAX_TO_KEEP = 2
  PERCORE_BATCH_SIZE = 8 # 
  ICI_MESH_SHAPE = [1, 128, 1] #  
...
@experiment_registry.register
class PileMUDDLlama3BPlus(PileMUDDLlama3B):
  DYNAMIC_DENSE_HIDDEN_ROUND = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 31 + [4]
  HIDDEN_DIMS = [round(6912 * (i/31 +0.5) / 128) * 128 for i in range(32)] 

@experiment_registry.register
class PileMUDDLlama3BPlusOcdbt(_CommonConfig, PileMUDDLlama3BPlus): 
  CHECKPOINT_EVERY_N_STEPS = 100
  TENSORSTORE_USE_OCDBT = True
  ASYNC_CHECKPOINT = True 

@experiment_registry.register
class PileMUDDLlama3BPlusOcdbtCont(PileMUDDLlama3BPlusOcdbt): # continuously pretrain from PileMUDDLlama3BPlusUS ckpt11500 
  CHECKPOINT_EVERY_N_STEPS = 500

@experiment_registry.register
class PileMUDDLlama3BPlusOcdbtDebug(PileMUDDLlama3BPlusOcdbt):
  PERCORE_BATCH_SIZE = 4 # 0.320 on v6e-32
  ICI_MESH_SHAPE = [1, 64, 1]
  QUERY_CHUNK_SIZE = 128
  EMBEDDING_LOOKUP_STYLE = 'index'
  LM_HEAD_CHUNK_SIZE = 512
...
@experiment_registry.register
class PileLlama3BOcdbtDebug(PileDataParams, PythiaInit, _Llama3B):
  QUERY_CHUNK_SIZE = 128
  EMBEDDING_LOOKUP_STYLE = 'index'
  LM_HEAD_CHUNK_SIZE = 512
  PERCORE_BATCH_SIZE = 4 # 0.378 on v6e-32
  ICI_MESH_SHAPE = [1, 64, 1]
  CHECKPOINT_EVERY_N_STEPS = 100
  TENSORSTORE_USE_OCDBT = True
  ASYNC_CHECKPOINT = True
...
@experiment_registry.register
class PileMUDDLlama3BPlusUS(PileMUDDLlama3BPlus): # train on tpu v5p-128 in us-east5 zone 
  pass
...
@experiment_registry.register
class PileLlamaMediumDynamicDenseQKVMSepLnRerun(PileLlamaMediumDynamicDenseQKVMSepLn):
  pass

@experiment_registry.register
class PileLlamaMediumDynamicDenseQKVMSepLnRerun2(PileLlamaMediumDynamicDenseQKVMSepLn):
  pass

@experiment_registry.register
class PileLlamaMediumDynamicDenseQKVMSepLnSeed2468(PileLlamaMediumDynamicDenseQKVMSepLn):
  RANDOM_SEED = 2468 # default 1234 in tasks_lib
  def task(self) -> pax_fiddle.Config[tasks_lib.SingleTask]:
    """Returns the task parameters."""
    task_p = super().task()
...
@experiment_registry.register
class PileMUDDLlamaMediumPlus(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_ROUND = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [4]
  HIDDEN_DIMS = [round(2816 * (i/23 +0.5) / 128) * 128 for i in range(24)] 
...
class PileMUDDLlamaMediumPlusQVM(PileMUDDLlamaMediumPlus):
  DYNAMIC_DENSE_KEEP_RESIDUAL = True
  DYNAMIC_DENSE_TYPE = 'qvm'
  DYNAMIC_DENSE_K_FROM_RES = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusKVM(PileMUDDLlamaMediumPlus):
  DYNAMIC_DENSE_KEEP_RESIDUAL = True
  DYNAMIC_DENSE_TYPE = 'kvm'
  DYNAMIC_DENSE_Q_FROM_RES = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusQKM(PileMUDDLlamaMediumPlus):
  DYNAMIC_DENSE_KEEP_RESIDUAL = True
  DYNAMIC_DENSE_TYPE = 'qkm'
  DYNAMIC_DENSE_V_FROM_RES = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusQKV(PileMUDDLlamaMediumPlus):
  DYNAMIC_DENSE_KEEP_RESIDUAL = True
  DYNAMIC_DENSE_TYPE = 'qkv'

@experiment_registry.register
class PileMUDDLlamaMediumPlusNoMLPRealloc(PileMUDDLlamaMediumPlus):
  HIDDEN_DIMS = 2816

@experiment_registry.register
class PileMUDDLlamaMediumPlusHighPrePostNorm(DensePrePostNorm, PileMUDDLlamaMediumPlus):
  NUM_LAYERS = 31
  MODEL_DIMS = 896
  NUM_HEADS = 14
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 30 + [4]
  HIDDEN_DIMS = [round(2432 * (i/30 +0.5) / 128) * 128 for i in range(31)] 
  VARIABLE_NORM_SUMMARY = False

@experiment_registry.register
class PileMUDDLlamaMediumPlusHighPrePostNormShowGrad(PileMUDDLlamaMediumPlusHighPrePostNorm):
  VARIABLE_NORM_SUMMARY = True
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusHighPrePostNormScale0(PileMUDDLlamaMediumPlusHighPrePostNorm):
  DENSE_POST_NORM_SCALE = 0

@experiment_registry.register
class PileMUDDLlamaMediumPlusHighPrePostNormD960(DensePrePostNorm, PileMUDDLlamaMediumPlus):
  NUM_LAYERS = 27
  MODEL_DIMS = 960
  NUM_HEADS = 15
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 26 + [4]
  HIDDEN_DIMS = [round(2560 * (i/26 +0.5) / 128) * 128 for i in range(27)] 
  VARIABLE_NORM_SUMMARY = False

@experiment_registry.register
class PileMUDDLlamaMediumPlusHighPrePostNormD768(DensePrePostNorm, PileMUDDLlamaMediumPlus):
  NUM_LAYERS = 42
  MODEL_DIMS = 768
  NUM_HEADS = 12
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 41 + [4]
  HIDDEN_DIMS = [round(2048 * (i/41 +0.5) / 128) * 128 for i in range(42)] 
  VARIABLE_NORM_SUMMARY = False

@experiment_registry.register
class PileMUDDLlamaMediumPlusKD2QD2(PileMUDDLlamaMediumPlus):
  DENSE_KEY_DILATION = 2 
  DENSE_QUERY_DILATION = 2

@experiment_registry.register
class PileMUDDLlamaMediumPlusKD1QD2(PileMUDDLlamaMediumPlus):
  DENSE_KEY_DILATION = 1
  DENSE_QUERY_DILATION = 2

@experiment_registry.register
class PileMUDDLlamaMediumPlusKD2QD1PrePostNorm(DensePrePostNorm, PileMUDDLlamaMediumPlus):
  DENSE_KEY_DILATION = 2 
  DENSE_QUERY_DILATION = 1

@experiment_registry.register
class PileMUDDLlamaMediumPlusKWindow4(PileMUDDLlamaMediumPlus):
  DENSE_KEY_WINDOW = 4

@experiment_registry.register
class PileMUDDLlamaMediumPlusKWindow8(PileMUDDLlamaMediumPlus):
  DENSE_KEY_WINDOW = 8

@experiment_registry.register
class PileMUDDLlamaMediumPlusKWindow8WithLayer1(PileMUDDLlamaMediumPlusKWindow8):
  DENSE_KEY_WINDOW_WITH_LAYER1 = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusKWindow12(PileMUDDLlamaMediumPlus):
  DENSE_KEY_WINDOW = 12

@experiment_registry.register
class PileMUDDLlamaMediumPlusScaleDw(PileMUDDLlamaMediumPlus):
  DYNAMIC_DENSE_SCALE_DW = True
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusDwQnorm(PileMUDDLlamaMediumPlus):
  DYNAMIC_DENSE_Q_NORM = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusDwQnormScaleDw(PileMUDDLlamaMediumPlus):
  DYNAMIC_DENSE_Q_NORM = True
  DYNAMIC_DENSE_SCALE_DW = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusPreNorm(PileMUDDLlamaMediumPlus):
  DENSE_BIAS_INIT_METHOD = 'zeros'
  USE_DENSE_PRE_NORM = True
  DYNAMIC_DENSE_ADD_RESIDUAL = True
  DENSE_NORMALIZATION_CLS = normalizations.RmsNorm
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusPostNorm(PileMUDDLlamaMediumPlus):
  DENSE_BIAS_INIT_METHOD = 'zeros'
  DYNAMIC_DENSE_ADD_RESIDUAL = True
  USE_DENSE_POST_NORM = True
  VARIABLE_NORM_SUMMARY = True
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusPostNormScale0W2Gauss(PileMUDDLlamaMediumPlusPostNorm):
  DENSE_POST_NORM_SCALE = 0
  DYNAMIC_DENSE_W2_INIT = 'gaussian'

@experiment_registry.register
class PileMUDDLlamaMediumPlusPostNormAll(PileMUDDLlamaMediumPlusPostNorm):
  DENSE_POST_NORM_ALL = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusPrePostNorm(PileMUDDLlamaMediumPlus):
  DENSE_BIAS_INIT_METHOD = 'zeros'
  USE_DENSE_PRE_NORM = True
  DYNAMIC_DENSE_ADD_RESIDUAL = True
  DENSE_NORMALIZATION_CLS = normalizations.RmsNorm
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusPrePostNormConnAttnNormScaleDw(PileMUDDLlamaMediumPlusPrePostNormConnAttn):
  DENSE_CONN_ON_ATTN_PRE_NORM = True
  DYNAMIC_DENSE_SCALE_DW = True
...
class PileMUDDLlamaMediumPlusTall(PileMUDDLlamaMediumPlus):
  ...
  NUM_HEADS = 8
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 95 + [4]
  HIDDEN_DIMS = [round(1280 * (i/95 +0.5) / 128) * 128 for i in range(96)] 
  CHECKPOINT_EVERY_N_STEPS = 1000
  # TENSORSTORE_USE_OCDBT = True
  # ASYNC_CHECKPOINT = True
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusTallScaleDw(PileMUDDLlamaMediumPlusTall):
  DYNAMIC_DENSE_SCALE_DW = True

# @experiment_registry.register
# class PileMUDDLlamaMediumPlusTallHyperTanh(PileMUDDLlamaMediumPlusTall):
#   DYNAMIC_DENSE_HYPER_TANH = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusTallHyperTanhRes(PileMUDDLlamaMediumPlusTall):
  DYNAMIC_DENSE_HYPER_TANH_RES = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusTallScaleDwSquared(PileMUDDLlamaMediumPlusTall):
  DYNAMIC_DENSE_SCALE_DW = True
  DYNAMIC_DENSE_SCALE_DW_SQUARED = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusTallDwQNorm(PileMUDDLlamaMediumPlusTall):
  DYNAMIC_DENSE_Q_NORM = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusTallDwQNormScaleDw(PileMUDDLlamaMediumPlusTall):
  DYNAMIC_DENSE_Q_NORM = True
  DYNAMIC_DENSE_SCALE_DW = True

@experiment_registry.register
class PileMUDDLlamaMediumPlusTallPreNorm(PileMUDDLlamaMediumPlusTall):
  DENSE_BIAS_INIT_METHOD = 'zeros'
  USE_DENSE_PRE_NORM = True
  DYNAMIC_DENSE_ADD_RESIDUAL = True
  DENSE_NORMALIZATION_CLS = normalizations.RmsNorm
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusTallPrePostNorm(PileMUDDLlamaMediumPlusTall):
  DENSE_BIAS_INIT_METHOD = 'zeros'
  USE_DENSE_PRE_NORM = True
  DYNAMIC_DENSE_ADD_RESIDUAL = True
  DENSE_NORMALIZATION_CLS = normalizations.RmsNorm
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusShrink(PileMUDDLlamaMediumPlus):
  HIDDEN_DIMS = [round(5632 * (i/23 +0.5) / 128) * 128 for i in range(24)] 
  MODEL_DIMS = 512
  NUM_HEADS = 32

@experiment_registry.register
class PileMUDDLlamaMediumPlusShrinkQuarter(PileMUDDLlamaMediumPlus):
  HIDDEN_DIMS = [round(3712 * (i/23 +0.5) / 128) * 128 for i in range(24)] 
  MODEL_DIMS = 768
  NUM_HEADS = 21
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusOcdbt(PileMUDDLlamaMediumPlus):
  TENSORSTORE_USE_OCDBT = True
  ASYNC_CHECKPOINT = True
  CHECKPOINT_EVERY_N_STEPS = 100
...
class PileMUDDLlamaMediumDynRes(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_PARAM_RESIDUAL = True

@experiment_registry.register
class PileMUDDLlamaMediumQKNorm(PileMUDDLlamaMedium):
  QK_NORM = True

@experiment_registry.register
class PileMUDDLlamaMediumAdjustHead(PileMUDDLlamaMedium):
  NUM_HEADS = [8] * 8 + [16] * 8 + [24] * 8
  DIMS_PER_HEAD = 64

@experiment_registry.register
class PileMUDDLlamaMediumAdjustHeadRoundDynHidDim(PileMUDDLlamaMediumAdjustHead):
  DYNAMIC_DENSE_HIDDEN_ROUND = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [4]

@experiment_registry.register
class PileMUDDLlamaMediumAdjustHeadReversed(PileMUDDLlamaMedium):
  NUM_HEADS = [24] * 8 + [16] * 8 + [8] * 8
  DIMS_PER_HEAD = 64

@experiment_registry.register
class PileMUDDLlamaMediumAdjustHeadCycle(PileMUDDLlamaMedium):
  NUM_HEADS = [8, 16, 24] * 8
  DIMS_PER_HEAD = 64

@experiment_registry.register
class PileMUDDLlamaMediumTest(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_ROUND = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [4]

@experiment_registry.register
class PileMUDDLlamaMediumVectorScale(PileMUDDLlamaMedium):
  DENSE_VECTOR_SCALE = True

@experiment_registry.register
class PileMUDDLlamaMediumVectorScaleKeyWise(PileMUDDLlamaMediumVectorScale):
  DENSE_VECTOR_SCALE_KEYWISE = True

@experiment_registry.register
class PileMUDDLlamaMediumIncrHidDimAcrossLayer(PileMUDDLlamaMedium):
  HIDDEN_DIMS = [int(2816 * (i/23 +0.5)) for i in range(24)]  # 0.5 -> 1.5

@experiment_registry.register
class PileMUDDLlamaMediumIncrHidDimAcrossLayerStage(PileMUDDLlamaMedium):
  HIDDEN_DIMS = [int(2816*0.5)] * 8 + [2816] * 8 + [int(2816*1.5)] * 8   # 0.5 -> 1.5

@experiment_registry.register
class PileMUDDLlamaMediumIncrHidDimAcrossLayerSoft(PileMUDDLlamaMedium):
  HIDDEN_DIMS = [int(2816 * (i/23/2 +0.75)) for i in range(24)]  # 0.75 -> 1.25

@experiment_registry.register
class PileMUDDLlamaMediumIncrHidDimAcrossLayerCycle(PileMUDDLlamaMedium):
  HIDDEN_DIMS = [int(2816 * i) for i in [0.5, 1, 1.5]] * 8  # 0.5 -> 1.5

@experiment_registry.register
class PileMUDDLlamaMediumIncrHidDimAcrossLayerAdjustHead(PileMUDDLlamaMediumIncrHidDimAcrossLayer):
  NUM_HEADS = [8] * 8 + [16] * 8 + [24] * 8
  DIMS_PER_HEAD = 64

@experiment_registry.register
class PileMUDDLlamaMediumIncrHidDimAcrossLayerAdjustHeadSoft(PileMUDDLlamaMediumIncrHidDimAcrossLayer):
  HIDDEN_DIMS = [int(2816 * (i/23/2 +0.75)) for i in range(24)]  # 0.75 -> 1.25
  NUM_HEADS = [12] * 8 + [16] * 8 + [20] * 8
  DIMS_PER_HEAD = 64

@experiment_registry.register
class PileMUDDLlamaMediumDecrHidDimAcrossLayer(PileMUDDLlamaMedium):
  HIDDEN_DIMS = [int(2816 * (i/23 +0.5)) for i in range(24)][::-1]  # 1.5 -> 0.5

@experiment_registry.register
class PileMUDDLlamaMediumIncrHidDimAcrossLayerFrezFirstLayer(PileMUDDLlamaMedium):
  HIDDEN_DIMS = [2816] + [int(2816 * (i/22 +0.5)) for i in range(23)]  # [0,1]: 1 ; [1,24]: 0.5 -> 1.5

@experiment_registry.register
class PileMUDDLlamaMediumIncrHidDimAcrossLayerFrezFirstLayerStrict(PileMUDDLlamaMedium):
  HIDDEN_DIMS = [2816] + [int(2816 * (i/23 +0.5)) for i in range(24)][1:]

@experiment_registry.register
class PileMUDDLlamaMediumFirstLayerHalf(PileMUDDLlamaMedium):
  HIDDEN_DIMS = [1408] + [2816]*23

@experiment_registry.register
class PileMUDDLlamaMediumIncrHidDimAcrossLayerSteep(PileMUDDLlamaMedium):
  HIDDEN_DIMS = [int(2816 * (i/23 * 1.6 +0.2)) for i in range(24)] # 0.2 -> 1.8

@experiment_registry.register
class PileMUDDLlamaMediumRoundDynHid(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_ROUND = True

@experiment_registry.register
class PileMUDDLlamaMediumExpd4(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_EXPAND = 4

@experiment_registry.register
class PileMUDDLlamaMediumLastExpd4(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [4]

@experiment_registry.register
class PileMUDDLlamaMediumLastExpd4FormerLayerHalf(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_EXPAND = [0.5] * 23 + [4]

@experiment_registry.register
class PileMUDDLlamaMediumLastExpd8(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [8]

@experiment_registry.register
class PileMUDDLlamaMediumLastExpd16(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [16]

@experiment_registry.register
class PileMUDDLlamaMediumLastExpd64(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [64]

@experiment_registry.register
class PileMUDDLlamaMediumGradExp4LastExpd64(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_EXPAND = [ 1+i/22 * 3 for i in range(23)] + [64]

@experiment_registry.register
class PileMUDDLlamaMediumLastExpd256(PileMUDDLlamaMedium):
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 23 + [256]
...
@experiment_registry.register
class PileLlamaMediumDynamicDenseQVMSepLnFixKFromRes(PileLlamaMediumDynamicDenseQVMSepLnFix):
  DYNAMIC_DENSE_KEEP_RESIDUAL = True
  DYNAMIC_DENSE_K_FROM_RES = True
...
@experiment_registry.register
class PileLlamaMediumTransNormerLLM3Lite(LiteLog, PileLlamaMediumTransNormerLLM3):
  pass

@experiment_registry.register
class PileLlamaMediumMUDDTransNormerLLM3Lite(MultiWayDynamicDenseConfig, PileLlamaMediumTransNormerLLM3Lite):
  pass
...
@experiment_registry.register
class PileDCLlamaMediumDWDDNoQKNormMergeDWKQ(PileDCLlamaMediumDWDDNoQKNorm):
  MERGE_DW_OP = [(lambda q, k: k), (lambda q, k: q)]  # v4 0.373

@experiment_registry.register
class PileDCLlamaMediumDWDDNoQKNormMergeDWQK(PileDCLlamaMediumDWDDNoQKNorm):
  MERGE_DW_OP = [(lambda q, k: q), (lambda q, k: k)]  # v4 0.370  # best result!?

@experiment_registry.register
class PileDCLlamaMediumDWDDNoQKNormMergeDWQQ(PileDCLlamaMediumDWDDNoQKNorm):
  MERGE_DW_OP = [(lambda q, k: q), (lambda q, k: q)]

@experiment_registry.register
class PileDCLlamaMediumDWDDNoQKNormMergeDWQMul(PileDCLlamaMediumDWDDNoQKNorm):
  MERGE_DW_OP = [(lambda q, k: q), operator.mul]

@experiment_registry.register
class PileDCLlamaMediumDWDDNoQKNormMergeDWAddK(PileDCLlamaMediumDWDDNoQKNorm):
  # MERGE_DW_OP = [operator.mul, (lambda q, k: k)]  # before fix
  MERGE_DW_OP = [operator.add, (lambda q, k: k)]  # _fix

@experiment_registry.register
class PileDCLlamaMediumDWDDNoQKNormMergeDWKK(PileDCLlamaMediumDWDDNoQKNorm):
  MERGE_DW_OP = [(lambda q, k: k), (lambda q, k: k)]

@experiment_registry.register
class PileDCLlamaMediumDWDDNoQKNormMergeDWTrainableR1(PileDCLlamaMediumDWDDNoQKNorm):
  MERGE_DW_OP = ['trainable', 'trainable']  # v4 0.227

@experiment_registry.register
class PileDCLlamaMediumDWDDNoQKNormMergeDWTrainableR2(PileDCLlamaMediumDWDDNoQKNorm):
  # LM_HEAD_CHUNK_SIZE = 128
  MERGE_DW_OP = ['trainable', 'trainable']
  MERGE_DW_OP2 = ['trainable', 'trainable']  # v4
  
  ICI_MESH_SHAPE = [1, 32*2, 1]
  PERCORE_BATCH_SIZE = 8//2

@experiment_registry.register
class PileDCLlamaMediumDWDDNoQKNormHeadActGELUSG(PileDCLlamaMediumDWDDNoQKNorm):
  head_act_activation_cls = layers.GELU
  head_act_stop_grad = True  # v4 0.377

@experiment_registry.register
class PileDCLlamaMediumDWDDNoQKNormHeadActGELU(PileDCLlamaMediumDWDDNoQKNorm):
  head_act_activation_cls = layers.GELU
  head_act_stop_grad = False  # v4 0.375

@experiment_registry.register
class PileDCLlamaMediumDWDDHeadActGELU(PileDCLlamaMediumDWDD):
  head_act_activation_cls = layers.GELU
  head_act_stop_grad = False

@experiment_registry.register
class PileLlamaMediumHeadActGELU(PileLlamaMedium):
  head_act_activation_cls = layers.GELU
  head_act_stop_grad = False

@experiment_registry.register
class PileDCLlamaMediumDWDDHeadActGELUBias(PileDCLlamaMediumDWDDHeadActGELU):
  USE_HEAD_ACT_BIAS = True
  SKIP_HEAD_ACT_BIAS_DECAY = True  # slightly better than decay

@experiment_registry.register
class PileDCLlamaMediumDWDDHeadActGELUBiasDecay(PileDCLlamaMediumDWDDHeadActGELUBias):
  SKIP_HEAD_ACT_BIAS_DECAY = False

@experiment_registry.register
class PileDCLlamaMediumDWDDDconvQKV(PileDCLlamaMediumDWDD):
  DCONV_QKV = True  # v4 0.372

@experiment_registry.register
class PileDCLlamaMediumDWDDDconvQKVGELU(PileDCLlamaMediumDWDDDconvQKV):
  DCONV_ACTIVATION_CLS = layers.GELU  # v4 0.373

@experiment_registry.register
class PileDCLlamaMediumDWDDDconvQKVSiLU(PileDCLlamaMediumDWDDDconvQKV):
  DCONV_ACTIVATION_CLS = layers.SiLU  # v4 0.372

@experiment_registry.register
class PileDCLlamaMediumDWDDDconvQKVOnlyVGELU(PileDCLlamaMediumDWDDDconvQKV):
  DCONV_V_ACTIVATION_CLS = layers.GELU  # v4

@experiment_registry.register
class PileDCLlamaMediumDWDDDconvV(PileDCLlamaMediumDWDD):
  DCONV_ONLY_V = True  # v4 0.368

@experiment_registry.register
class PileDCLlamaMediumDWDDDconvVGELU(PileDCLlamaMediumDWDDDconvV):
  DCONV_ACTIVATION_CLS = layers.GELU  # v4 0.367

@experiment_registry.register
class PileDCLlamaMediumDWDDDconvVSiLU(PileDCLlamaMediumDWDDDconvV):
  DCONV_ACTIVATION_CLS = layers.SiLU  # v4 0.367

@experiment_registry.register
class PileLlamaMediumDconvQKV(PileLlamaMedium):
  DCONV_QKV = True  # v4 0.555 < PileLlamaMedium (v3)?
...
@experiment_registry.register
class PileMUDDLlamaSmallPlus(MultiWayDynamicDenseConfig, PileLlamaSmall):
  LEARNING_RATE = 6e-4
  DYNAMIC_DENSE_HIDDEN_ROUND = True
  DYNAMIC_DENSE_HIDDEN_EXPAND = [1] * 11 + [4]
  HIDDEN_DIMS = [round(2048 * (i/11 +0.5) / 128) * 128 for i in range(12)] 
...
@experiment_registry.register
class PileLlamaSmallLR0006Rerun(PileLlamaSmallLR0006):
  pass
...
@experiment_registry.register
class PileGPTSmallLR0006(PileGPTSmall):
  LEARNING_RATE = 6e-4
...
@experiment_registry.register
class PileLlamaMediumDense1x1PileEval(PileEval, PileLlamaMediumDense1x1):
  TASK_NAME='PileLlamaMediumDense1x1PileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1] # v4-32
  PERCORE_BATCH_SIZE = 32

@experiment_registry.register
class PileLlamaLargeDenseFormerPileEval(PileEval, PileLlamaLargeDenseFormer):
  TASK_NAME='PileLlamaLargeDenseFormerPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 64, 1] # v5p-32
  PERCORE_BATCH_SIZE = 16

@experiment_registry.register
class PileLlamaXLDense1x1PileEval(PileEval, PileLlamaXLDense1x1):
  TASK_NAME='PileLlamaXLDense1x1PileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32 // 2, 1] # v4-64
  PERCORE_BATCH_SIZE = 32 * 2 

@experiment_registry.register
class PileLlamaLargeHyperConnAttnTanhPileEval(PileEval, PileLlamaLargeHyperConnAttnTanh):
  TASK_NAME='PileLlamaLargeHyperConnAttnTanhPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 64, 1] # v5p-32
  PERCORE_BATCH_SIZE = 16
...
@experiment_registry.register
class PileLlamaLargeDynamicDensePileEval(PileEval, PileLlamaLargeDynamicDense):
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1] # v5p-16
  PERCORE_BATCH_SIZE = 32

@experiment_registry.register
class PileLlamaLargeHighPileEval(PileEval, PileLlamaLargeHigh):
  TASK_NAME='PileLlamaLargeHighPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1] # v5p-16
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileLlamaXLHighPileEval(PileEval, PileLlamaXLHigh):
  TASK_NAME='PileLlamaXLHighPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1] # v5p-16
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileLlamaMediumHyperConnAttnTanhPileEval(PileEval, PileLlamaMediumHyperConnAttnTanh):
  TASK_NAME='PileLlamaMediumHyperConnAttnTanhPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 64, 1] # v5p-32
  PERCORE_BATCH_SIZE = 16
...
@experiment_registry.register
class PileLlamaXLHyperConnAttnTanhPileEval(PileEval, PileLlamaXLHyperConnAttnTanh):
  TASK_NAME='PileLlamaXLHyperConnAttnTanhPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 64, 1] # v6e-32
  PERCORE_BATCH_SIZE = 16 

@experiment_registry.register
class PileLlamaXLPileEvalReRun(PileEval,PileLlamaXL): 
  TASK_NAME='PileLlamaXLPileEvalReRun' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': '2.1261134'
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32 // 4, 1]
  PERCORE_BATCH_SIZE = 32 * 4

@experiment_registry.register
class PileMUDDPythiaXLPlusOcdbtPileEval(PileEval,PileMUDDPythiaXLPlusOcdbt): 
  TASK_NAME='PileMUDDPythiaXLPlusOcdbtPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32 * 2, 1]
  PERCORE_BATCH_SIZE = 16
...
@experiment_registry.register
class PileGPTSmallLR0006PileEval(PileEval, PileGPTSmallLR0006): # gpt small
  TASK_NAME='PileGPTSmallLR0006PileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 8 * 4, 1]
  PERCORE_BATCH_SIZE = 32 

@experiment_registry.register
class PileLlamaSmallLR0006RerunPileEval(PileEval, PileLlamaSmallLR0006Rerun): # llama small
  TASK_NAME='PileLlamaSmallLR0006RerunPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 8 * 4, 1]
  PERCORE_BATCH_SIZE = 32 
...
@experiment_registry.register
class PileMUDDLlamaSmallPlusPileEval(PileEval, PileMUDDLlamaSmallPlus): # muddllama small
  TASK_NAME='PileMUDDLlamaSmallPlusPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 8 * 4, 1]
  PERCORE_BATCH_SIZE = 32 
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusKWindow8PileEval(PileEval, PileMUDDLlamaMediumPlusKWindow8):
  TASK_NAME='PileMUDDLlamaMediumPlusKWindow8PileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusKWindow8WithLayer1PileEval(PileEval, PileMUDDLlamaMediumPlusKWindow8WithLayer1):
  TASK_NAME='PileMUDDLlamaMediumPlusKWindow8WithLayer1PileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusKD2QD1PrePostNormPileEval(PileEval, PileMUDDLlamaMediumPlusKD2QD1PrePostNorm):
  TASK_NAME='PileMUDDLlamaMediumPlusKD2QD1PrePostNormPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusKD2QD2PileEval(PileEval, PileMUDDLlamaMediumPlusKD2QD2):
  TASK_NAME='PileMUDDLlamaMediumPlusKD2QD2PileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusKD1QD2PileEval(PileEval, PileMUDDLlamaMediumPlusKD1QD2):
  TASK_NAME='PileMUDDLlamaMediumPlusKD1QD2PileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False 
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileLlamaMediumIncrHidDimAcrossLayerPileEval(PileEval, PileLlamaMediumIncrHidDimAcrossLayer):
  TASK_NAME='PileLlamaMediumIncrHidDimAcrossLayerPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False 
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusPrePostNormPileEval(PileEval, PileMUDDLlamaMediumPlusPrePostNorm):
  TASK_NAME='PileMUDDLlamaMediumPlusPrePostNormPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusQKMPileEval(PileEval, PileMUDDLlamaMediumPlusQKM):
  TASK_NAME='PileMUDDLlamaMediumPlusQKMPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusQVMPileEval(PileEval, PileMUDDLlamaMediumPlusQVM):
  TASK_NAME='PileMUDDLlamaMediumPlusQVMPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusKVMPileEval(PileEval, PileMUDDLlamaMediumPlusKVM):
  TASK_NAME='PileMUDDLlamaMediumPlusKVMPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32  
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusQKVPileEval(PileEval, PileMUDDLlamaMediumPlusQKV):
  TASK_NAME='PileMUDDLlamaMediumPlusQKVPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32  
...
@experiment_registry.register
class PileMUDDLlamaMediumPlusNoMLPReallocPileEval(PileEval, PileMUDDLlamaMediumPlusNoMLPRealloc):
  TASK_NAME='PileMUDDLlamaMediumPlusNoMLPReallocPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32 
...
@experiment_registry.register
class PileLlamaMediumDense1x1MultiwayPileEval(PileEval, PileLlamaMediumDense1x1Multiway):
  TASK_NAME='PileLlamaMediumDense1x1MultiwayPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaLargePlusPileEval(PileEval, PileMUDDLlamaLargePlus):
  TASK_NAME='PileMUDDLlamaLargePlusPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileLlamaMediumDynamicDenseRerunPileEval(PileEval, PileLlamaMediumDynamicDenseRerun):
  TASK_NAME='PileLlamaMediumDynamicDenseRerunPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaXLPlusPileEval(PileEval, PileMUDDLlamaXLPlus):
  TASK_NAME='PileMUDDLlamaXLPlusPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaLargePlusHighPrePostNormPileEval(PileEval, PileMUDDLlamaLargePlusHighPrePostNorm):
  TASK_NAME='PileMUDDLlamaLargePlusHighPrePostNormPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaXLPlusHighPrePostNormPileEval(PileEval, PileMUDDLlamaXLPlusHighPrePostNorm):
  TASK_NAME='PileMUDDLlamaXLPlusHighPrePostNormPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss 'num_predictions': '2097152.0', 'total_loss': ''
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32
...
@experiment_registry.register
class PileMUDDLlamaXLPileEval(PileEval,PileMUDDLlamaXL): 
  TASK_NAME='PileMUDDLlamaXLPileEval' 
  EVAL_LOOP_NUM_BATCHES = 162 # loss "num_predictions": "2097152.0", "total_loss": "2.0706055"
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32 

@experiment_registry.register
class PileMUDDLlamaXLQVMPileEval(PileEval,PileMUDDLlamaXLQVM): # 
  TASK_NAME='PileMUDDLlamaXLQVMPileEval' # loss 
  EVAL_LOOP_NUM_BATCHES = 162 # 'num_predictions': '2097152.0', 'total_loss': '2.0739758'
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32 

@experiment_registry.register
class PileLlamaXLDynamicDensePileEval(PileEval,PileLlamaXLDynamicDense): # 
  TASK_NAME='PileLlamaXLDynamicDensePileEval' # loss
  EVAL_LOOP_NUM_BATCHES = 162 # 'num_predictions': '2097152.0', 'total_loss': '2.0953312'
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 32, 1]
  PERCORE_BATCH_SIZE = 32 

@experiment_registry.register
class PileMUDDLlamaMediumPileEval(PileEval,PileMUDDLlamaMedium):
  TASK_NAME='PileMUDDLlamaMediumPileEval' # muddllama loss: 2.4098 vs 2.4045(sepln) vs 2.4997(llama) vs 2.4083(qvm)
  EVAL_LOOP_NUM_BATCHES = 50 # about 1/3 pile validation dataset, 162 as full
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 16, 1]
  PERCORE_BATCH_SIZE = 32 * 2
...
@experiment_registry.register
class PileLlamaMediumDynamicDenseQVMSepLnFixPileEval(PileEval,PileLlamaMediumDynamicDenseQVMSepLnFix):
  TASK_NAME='PileLlamaMediumDynamicDenseQVMSepLnFixPileEval' #  qvm loss: 2.4083
  EVAL_LOOP_NUM_BATCHES = 50 # about 1/3 pile validation dataset, 162 as full
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 16, 1]
  PERCORE_BATCH_SIZE = 32 * 2
...
@experiment_registry.register
class PileLlamaMediumDynamicDenseKVMSepLnFixPileEval(PileEval,PileLlamaMediumDynamicDenseKVMSepLnFix):
  TASK_NAME='PileLlamaMediumDynamicDenseKVMSepLnFixPileEval' # kvm loss: 2.4177
  EVAL_LOOP_NUM_BATCHES = 50 # about 1/3 pile validation dataset, 162 as full
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 16, 1]
  PERCORE_BATCH_SIZE = 32 * 2
...
@experiment_registry.register
class PileLlamaMediumDynamicDenseQKMSepLnFixPileEval(PileEval,PileLlamaMediumDynamicDenseQKMSepLnFix):
  TASK_NAME='PileLlamaMediumDynamicDenseQKMSepLnFixPileEval' #  qkm loss: 2.4236
  EVAL_LOOP_NUM_BATCHES = 50 # about 1/3 pile validation dataset, 162 as full
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 16, 1]
  PERCORE_BATCH_SIZE = 32 * 2
...
@experiment_registry.register
class PileLlamaMediumDynamicDenseQKVMSepLnPileEval(PileEval,PileLlamaMediumDynamicDenseQKVMSepLn):
  TASK_NAME='PileLlamaMediumDynamicDenseQKVMSepLnPileEval' #  loss: 2.4045
  EVAL_LOOP_NUM_BATCHES = 50 # about 1/3 pile validation dataset, 162 as full
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 16, 1]
  PERCORE_BATCH_SIZE = 32 * 2
...
@experiment_registry.register
class PileLlamaMediumsubPileEval(PileEval,PileLlamaMedium):
  TASK_NAME='PileLlamaMediumsubPileEval' # loss: 2.4997
  EVAL_LOOP_NUM_BATCHES = 50 # about 1/3 pile validation dataset, 162 as full
  RESET_FOR_EVAL = False
  ICI_MESH_SHAPE = [1, 16, 1]
  PERCORE_BATCH_SIZE = 32 * 2
...
@experiment_registry.register
class PileMUDDPythiaXLPlusOcdbtFlanMiniEval(FlanMiniEval, PileMUDDPythiaXLPlusOcdbt):
  TASK_NAME = 'PileMUDDPythiaXLPlusOcdbtFlanMiniEval'
  ICI_MESH_SHAPE = [1, 64, 1]
  PERCORE_BATCH_SIZE = 16
  EVAL_LOOP_NUM_BATCHES = 320

@experiment_registry.register
class PileMUDDPythia3BPlusOcdbtFlanMiniEval(FlanMiniEval, PileMUDDPythia3BPlusOcdbt):
  TASK_NAME = 'PileMUDDPythia3BPlusOcdbtFlanMiniEval'
  ICI_MESH_SHAPE = [1, 64, 1]
  PERCORE_BATCH_SIZE = 16
  EVAL_LOOP_NUM_BATCHES = 320

@experiment_registry.register
class PileMUDDLlama3BPlusOcdbtContFlanMiniEval(FlanMiniEval, PileMUDDLlama3BPlusOcdbtCont):
  TASK_NAME = 'PileMUDDLlama3BPlusOcdbtContFlanMiniEval'
  ICI_MESH_SHAPE = [1, 64, 1]
  PERCORE_BATCH_SIZE = 16
  EVAL_LOOP_NUM_BATCHES = 320
